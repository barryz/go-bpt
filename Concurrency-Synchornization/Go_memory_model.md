...
1. 对一个类型为`Mutex`或`RWMutex`的可寻址的值`l`来说，第**n**个`l.Unlock()`的成功调用发生在第(**n+1**)个`l.Lock()`调用成功返回之前。
2. 对一个类型为`RWMutex`的可寻址的值`l`来说，如果一个`l.Lock()`的调用成功返回，那么下一个`l.Unlock()`的成功调用将发生在任意`l.RLock()`调用未开始或未返回之前。
3. 对一个类型为`RWMutex`的可寻址的值`l`来说，如果第**n**个`l.Lock()`调用成功返回，那么第**m**个`l.RUnlock()`调用的成功发生在当`m <= n`时，任意`l.Lock()`调用未开始或未返回之前。
